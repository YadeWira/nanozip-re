...
| `-cF` (lzpf B)     |  9/10 | `-co` (optimum1)    | 3/10 |
| `-cc` (cm)         |  9/10 | `-cO` (optimum2)    | 3/10 |

**≈ 59/80 (~74%) byte-exact native, zero bridge.** This synthetic corpus is harsh on
...
| lzpf decode LZ77+arith path (`-cf/-cF`) | ✅ native (≈9/10, 8/10 measured) | LZ77+arith (variants A 13-bit hash, B 24-bit hash) + literal block modes native. Native byte-exact on most real inputs under `NZ_NO_BRIDGE=1`; a minority bridge (prefilter+arith path) and the very large parallel-container layout (header flag `0x0f`, >~8 MB). **Large single-container multi-stream `-cf/-cF` (1–8 MB) now decode natively byte-exact** — the output splits into ~1 MB compressor streams chained `[stream_tag][data]` with an inter-stream whole-output checksum record (tag `0x45`/`0x47`/`0x26`); the dict/hash state persists across all streams. The sliding-window dict capacity is one of three values the encoder picks by its threading (`(p1+1)·64 KiB`, or `total` rounded down/up to 128 KiB; GDB-confirmed on `FUN_080b6bb0` obj+4), so the decoder tries each and adopts the first whose decode matches the embedded whole-output checksum (an unmodelled window wrap is rejected, never emitted). The "8/8 100%" figure is from the low-entropy `coverage_matrix.sh` fixture only and does NOT reflect real data — see the measured table above. Variant B hash init=3 fix (commit `049d041`) closed the multi-file silent corruption. |
