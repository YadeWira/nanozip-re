...
| Console matrices and the pty prompt harness (`tests/parity/`) | identical except progress-redraw timing, the order of `Compressor` lines of parallel workers, `argv[0]` in the usage text, and the encode commands |
...
| Archives made by the **Windows** original, 8 codecs, and our Windows build's console against it (`tests/windows_original.sh`) | 24/24: contents, listings and restored modes, including the file attributes a Windows archive stores instead of POSIX permissions |
