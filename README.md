...
| **Encode**, `a -cn` against the original's `a` on the same inputs and switches (`tests/encode/oracle.sh`: file order in its four `-s` modes, `-r`, `-sp`, `-x` and unreadable files, `-nt`/`-np`/`-nm`/`-fo`, every checksum kind, block boundaries, 1 MB multi-block inputs, `-pN -t1` worker splits with slices, one-byte and empty inputs) | 51/51 archives byte-identical, each read by the other binary, 51/51 consoles identical once timing figures are removed; the compressed codecs still write the stub |
...
**Not there yet:** encode of the compressing codecs (`a -cf/-cF/-cd/-cD/-co/-cO/-cc`, `s`) and self-extractor creation (`w32c`); `a -cn`, single and `-pN`, is byte-exact. Known limits: the
