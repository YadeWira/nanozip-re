...
| One entry over 4 GB: the reporter's 4.6 GB `-cO` archive (a 4600 MB MPG), and a 4.5 GB entry of real corpus material written by the original with each of `-cn -cf -cF -cd -cD -cc` (`/mnt/IA_LAB/agentes/NZ-RE/{xman,big4g_multi}`, `NZ_BIG_OUT=… tests/huge_archive.sh`) | all seven `t` OK and extract **byte-identical**; `-cc` also clean under ASan over the whole decode. `-co` on the same 4.5 GB entry stops on a preexisting param15 defect that reproduces at 180 MB (not a size issue; recorded). Before this the 32-bit-wrapped post-filter caps called all of them corrupt past 4 GB |
...
**Not there yet:** encode of `-cO/-cc` and `s`, and self-extractor creation (`w32c`); `a -co` writes plain-LZ, exe-filtered, text-transformed and BWT blocks, with the param1 and param2 post-filters, param14 on BWT blocks and stored (param6 == 0) BWT blocks, byte-identical to the original (twelve of twelve text oracle inputs, 47 of 47 corpus XML/HTML files, 62 of 62 executables, 176 of 180 mixed corpus files -- 285 of 289) and declining none of those 289 inputs, but not yet writing the image or audio blocks, the stored LZ form, param15, or budgets above 16 MB; a block it cannot write is declined with a message and no archive is left behind, and every block it does write is proved readable by our own decoder before it is committed; `a -cn` (single and `-pN`), `a -cf/-cF`, `a -cd` and `a -cD` (plain data, the exe and block-RLE filters, the text pipeline with its word dictionary, the audio and image models, under `-t1`, at every `-m`) are byte-exact. `a` with one of the three unported compressors prints a one-line refusal and writes nothing (the decode phase's stub container, which the original could not read, is gone). Known limits: the
