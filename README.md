...
**Not there yet:** encode of `-cd/-cD/-co/-cO/-cc` and `s`, and self-extractor creation (`w32c`); `a -cn` (single and `-pN`) and `a -cf/-cF` (plain data, the audio prefilter and the image model, under `-t1`) are byte-exact. Known limits: the
