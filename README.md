...
|--------|-------------------|--------|-------------------|
| `-cn` (store)      |  9/10 | `-cd` (lzhd)        | 8/10 |
| `-cf` (lzpf A)     |  9/10 | `-cD` (lzhd big)    | 6/10 |
| `-cF` (lzpf B)     |  8/10 | `-co` (optimum1)    | 3/10 |
| `-cc` (cm)         |  9/10 | `-cO` (optimum2)    | 3/10 |

**≈ 55/80 (~69%) byte-exact native, zero bridge.** This synthetic corpus is harsh on
...
multi-chunk text (text_50k/repeat_100k/CRLF). The non-native cases fall to the extract
...
the text pipeline (param14, line-RLE, CRLF, word-dictionary) and multi-chunk text — and
bridge only on LARGE multi-region files (a chunk whose far-back match crosses the 64 KB ring
in a way the vtable window object resolves but a flat ring does not — being reverse
engineered) and rare CM/BWT sub-chunks; `-co`/`-cO`
...
- `lzhd` (`-cd/-cD`): the coroutine token-LZ is native incl. multi-chunk text (see the component table); the remaining bridge cases are **large multi-region files** (a chunk whose far-back match crosses the 64 KB ring — the binary resolves it through a vtable double-buffer window that a flat ring does not reproduce; being reverse engineered) and rare CM/BWT `-cd` sub-chunks.
