...
| `-cf` (lzpf A)     |  9/10 | `-cD` (lzhd big)    | 6/10 |
| `-cF` (lzpf B)     |  8/10 | `-co` (optimum1)    | 3/10 |
| `-cc` (cm)         |  9/10 | `-cO` (optimum2)    | 3/10 |

**≈ 56/80 (~70%) byte-exact native, zero bridge.** This synthetic corpus is harsh on
...
multi-stream files — and bridge only on the residual case where a multi-stream archive's
LZ window wraps under heavy repetition (caught by the decoder's checksum self-verify and
re-decoded byte-exact via the bridge) plus rare CM/BWT sub-chunks; `-co`/`-cO`
...
- `lzhd` (`-cd/-cD`): the coroutine token-LZ is native incl. multi-chunk text and large multi-stream files (see the component table). The LZ window is a single per-archive ring of size `(method_p1+1)·0x10000` (the lzpf dict-size rule — 64 KB/128 KB/192 KB/… up to multi-MB, modular wrap so non-power-of-two sizes work) that persists across the archive's 1 MB output streams. The only residual bridge case is a multi-stream archive whose ring wraps under **heavy repetition** (the exact wrap-eviction semantics in that corner are still being reverse engineered); the decoder self-verifies against the stored per-file checksum and falls back to a byte-exact bridge decode there. Plus rare CM/BWT `-cd` sub-chunks.
...
2. ✅ Done: the `-cd` cross-chunk / cross-stream LZ window. It is a single per-archive ring of size `(method_p1+1)·0x10000` (modular wrap) that persists across the archive's 1 MB output streams (`NzCdDecodeStream` threads the ring position and the file-absolute output offset). This made multi-chunk text and large multi-stream files (a 7-chunk source file; 2–3.5 MB code/text/base64) byte-exact native. Residual: the ring-wrap-under-heavy-repetition corner, which self-verifies against the stored checksum and bridges.
...
| lzpf prefilter+arith path (`-cf`) | ⚠️ mono native; stereo bridges | `FUN_080a5330` + LPC filter (`FUN_08095d90`) ported; **mono audio byte-exact**. Stereo inter-channel LMS predictor `FUN_08096e20` (2-stage cascaded sign-sign 4-tap) is now ported faithfully (scalar path) in `lzpf_arith.cpp::ApplyLmsInterChannel` and **verified byte-exact in isolation** by calling the real binary in-process under GDB (5 residual sets incl. ring-wrap and coeff-saturation; regression test `TestLmsInterChannel`). **End-to-end stereo still bridges**: tracing `stereo_lms_cf.nz` natively, the stereo prefilter block decodes the WAV header correctly but diverges at the first audio sample — `DecodePFBlock`'s stereo integration uses an interleaved channel split whereas `FUN_080a5330` is **planar** (ch1=`[0,n)`, ch2=`[n,2n)`), applies LPC per planar channel, then LMS, then `FUN_080a50c0`; and a second block uses the still-unported raw-bytecode LZ77 mode. Closing it needs that planar rework + the raw-bytecode mode (tracked). |
| lzhd decoder (`-cd/-cD`) | ✅ LZ + block-RLE + raw-store + pure-literal + exe + text-pipeline (param14/line-RLE/CRLF/word-dict) + multi-chunk text + large multi-stream files native; heavy-repetition ring-wrap corner bridges | The real linux32 `-cd` is a coroutine token-LZ (NOT the reference `DecLZ`). Fully ported in `nz_cd_tokens.{h,cpp}` and **validated byte-exact** against the binary: token assembler (`FUN_080aa070`), reconstruction with trailing-literal flush (`FUN_08099050`), per-column RLE run-expander (`FUN_080acb90`, thr=1 for the LEN column / 0 for LIT·OFF), param14 text transform (`FUN_080a0ff0`), the bounded-varint header (`FUN_080b1dc0`), and the integrated chunk/stream/block decoders (`NzCdDecodeLzChunk`/`NzCdDecodeStream`/`NzCdDecodeBlock`). Columns/literals reuse `DecodeArithBuffer`. The reconstruction runs over a **single per-archive ring** (`FUN_08099050`, obj+0x978) whose size is `(method_p1+1)·0x10000` — the same dict-size rule as lzpf (64 KB/128 KB/192 KB/… up to multi-MB). The size need not be a power of two, so the ring uses **modular wrap** (not a bitmask): each chunk's compact recon is written at the chunk's ring base, the base advances by the COMPACT recon size and resets to 0 when a chunk would not fit before the ring end, and matches resolve into prior chunks through the wrap (verified vs the binary's obj+0x980 across f18/text50/repeat/crlf and a 7-chunk 192 KB-ring source file). Large files split their output into **1 MB streams**; the ring is allocated once and **persists across streams** (`NzCdDecodeStream` threads `ring_pos` and the file-absolute output offset), so cross-stream matches resolve correctly. **Wired into the extract dispatcher** and BYTE-EXACT end-to-end under `NZ_NO_BRIDGE=1` for: token-LZ (recon == file), **block-RLE** (flag `&2`, post-recon run-length re-expansion of collapsed zero-runs), **raw-store** (per-column `b0&1==0` and flag-`&1`-clear literals = verbatim bytes), **pure-literal** (no LZ tokens — the whole window is one literal stream; generator picks this when `size_field==0` or `v2==0`), **exe** (flag `&4`: a BCJ-style x86 E8/E9 call/jmp address un-transform, `NzCdExeUnfilter`), and the **text pipeline** (flag `&8`, `FUN_080a3c90`): param14 (`NzCdParam14`), line-RLE (`FUN_080a2f20`), CRLF EOL (`FUN_080a19b0`), and the **word-dictionary** transform (`FUN_080a0a00` = the reference `TransformText_1_Dictionary` + dict/char-trait tables, in `nz_cd_texttransform_dict.cpp`). Verified on text (map.txt), binary (image.cat), an ELF (elf.bin, flags=3 block-RLE), an EXE (play.exe, flags=5), atoll (multi-chunk `&8`), word-dictionary text, multi-chunk text (text_50k/repeat_100k/CRLF), a 7-chunk source file (single-stream 192 KB ring), large multi-stream files (2–3.5 MB code/text/base64), and a varied real corpus spanning images/audio/video/executables. **Residual:** a multi-stream archive whose ring wraps under heavy repetition still diverges (the exact wrap-eviction semantics in that corner are being reverse engineered); `TryDecodeLegacyLzhd` **self-verifies the decoded output against the archive's stored per-file checksum** and returns false on mismatch, so that case falls through to a byte-exact bridge decode (no silent corruption, and `NZ_NO_BRIDGE` native-only becomes a provable correctness signal). The old `DecLZ` (`nz_lzhd.cpp`, nzdec_v0) is the wrong format for linux32 `-cd`. |
...
| 2026-06-20 | **`-cd` large-file LZ window solved.** The cross-chunk window is a single **per-archive ring** of size `(method_p1+1)·0x10000` (the lzpf dict-size rule), not a fixed 64 KB — confirmed via GDB (`ebp`/obj+0x978 = 65536 for f18/text50, 196608 for a 7-chunk source file, multi-MB for large files). Sizes are not powers of two, so the ring helpers were switched from bitmask to **modular wrap** (`nz_cd_tokens.cpp`). Large files split output into **1 MB streams** that match into each other through the ring, so the ring now **persists across streams**: new `NzCdDecodeStream` takes a caller-owned ring + `ring_pos` (in/out) + file-absolute output offset, and the dispatcher allocates it once. Made byte-exact native: a 7-chunk single-stream 192 KB-ring source file and 2–3.5 MB multi-stream code/text/base64 (`-cd` `native_only_v2` 8/10→9/10). `TryDecodeLegacyLzhd` now **self-verifies the decode against the stored per-file checksum** and returns false on mismatch, so the one residual corner (multi-stream ring-wrap under heavy repetition) falls back to a byte-exact bridge decode with no silent corruption. The earlier "vtable double-buffer window / fixed 64 KB ring" theory was superseded. |
