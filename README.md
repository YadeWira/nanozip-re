...
| lzhd decoder (`-cd/-cD`) | ✅ pure-LZ native (~50% of real files); rest bridge | The real linux32 `-cd` is a coroutine token-LZ (NOT the reference `DecLZ`). Fully ported in `nz_cd_tokens.{h,cpp}` and **validated byte-exact** against the binary: token assembler (`FUN_080aa070`), reconstruction (`FUN_08099050`), per-column RLE run-expander (`FUN_080acb90`), param14 text transform (`FUN_080a0ff0`), the bounded-varint header (`FUN_080b1dc0`), and the integrated chunk + whole-block decoders (`NzCdDecodeLzChunk`/`NzCdDecodeBlock`). Columns/literals reuse `DecodeArithBuffer`. **Wired into the extract dispatcher**: pure-LZ `-cd` files (recon output == file) decode BYTE-EXACT end-to-end under `NZ_NO_BRIDGE=1` — verified on text (map.txt) and binary (image.cat) and ~12/24 of a varied real corpus. The remaining files carry a post-recon **block-RLE** (chunk flag `&2`, same expander applied to the recon window) and still bridge until that flags=3 chunk layout is wired; CM/BWT chunk variants likewise. The old `DecLZ` (`nz_lzhd.cpp`, nzdec_v0) is the wrong format for linux32 `-cd`. |
