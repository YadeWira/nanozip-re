...
| `-cc` (cm)         |  9/10 | `-cO` (optimum2)    | 7/10 |

**≈ 71/80 (~89%) byte-exact native, zero bridge.** `-cn`/`-cf`/`-cF`/`-cc` are native for typical real inputs.
...
native for single-container AND parallel-container LZ/CM content, bridging on the still-unported `decr_param==0`
(BWT) sub-path. Counts vary ±1 because the corpus uses random fixtures.
