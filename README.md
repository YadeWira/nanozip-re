...
|--------|-------------------|--------|-------------------|
| `-cn` (store)      | 10/10 | `-cd` (lzhd)        | 7/10 |
| `-cf` (lzpf A)     |  9/10 | `-cD` (lzhd big)    | 7/10 |
| `-cF` (lzpf B)     |  8/10 | `-co` (optimum1)    | 3/10 |
| `-cc` (cm)         |  9/10 | `-cO` (optimum2)    | 3/10 |

**≈ 55/80 (~69%) byte-exact native, zero bridge.** The non-native cases fall to the
extract bridge (legacy `nz`) and are the known gaps below. `-cn`/`-cf`/`-cF`/`-cc`
are native for typical real inputs; `-cd`/`-cD` are native for LZ and block-RLE
chunks (~73% of a varied corpus) and bridge only on chunks that use the CM/BWT/
param14 sub-methods; `-co`/`-cO` (large) bridge on the virtual-stream LZ framing.
Counts vary ±1 because the corpus uses random fixtures.
