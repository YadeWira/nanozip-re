...
|--------|-------------------|--------|-------------------|
| `-cn` (store)      |  9/10 | `-cd` (lzhd)        | 6/10 |
| `-cf` (lzpf A)     |  9/10 | `-cD` (lzhd big)    | 6/10 |
| `-cF` (lzpf B)     |  8/10 | `-co` (optimum1)    | 3/10 |
| `-cc` (cm)         |  9/10 | `-cO` (optimum2)    | 3/10 |

**≈ 53/80 (~66%) byte-exact native, zero bridge.** This synthetic corpus is harsh on
`-cd`/`-cD`: three of its fixtures are *multi-chunk* text and one is stereo audio, all of
which still bridge — so 6/10 understates real-world `-cd`, which is byte-exact on a varied
real corpus (10/10 on the elf/map/image/atoll + f-series set used in the unit fixtures).
The non-native cases fall to the extract bridge (legacy `nz`) and are the known gaps below.
`-cn`/`-cf`/`-cF`/`-cc` are native for typical real inputs; `-cd`/`-cD` are native for LZ,
block-RLE, raw-store, pure-literal, exe, and the text pipeline (param14, line-RLE, CRLF,
word-dictionary) — and bridge only on **multi-chunk text** (the cross-chunk LZ window for a
chunk following a text chunk is not yet pinned) and rare CM/BWT sub-chunks; `-co`/`-cO`
...
- `lzhd` (`-cd/-cD`): the coroutine token-LZ is native (see the component table); the remaining bridge cases are **multi-chunk text** (a chunk following a text `&8` chunk — the cross-chunk ring advance for that case is unresolved) and rare CM/BWT `-cd` sub-chunks.
...
2. Pin the `-cd` **multi-chunk text** cross-chunk window: a chunk following a text (`&8`) chunk references that chunk's expanded output, and the ring advance for that case is not yet resolved (single-chunk text decodes byte-exact). Unblocks real multi-chunk text `-cd`/`-cD`.
...
| lzhd decoder (`-cd/-cD`) | ✅ LZ + block-RLE + raw-store + pure-literal + exe + text-pipeline (param14/line-RLE/CRLF/word-dict) native; multi-chunk text bridges | The real linux32 `-cd` is a coroutine token-LZ (NOT the reference `DecLZ`). Fully ported in `nz_cd_tokens.{h,cpp}` and **validated byte-exact** against the binary: token assembler (`FUN_080aa070`), reconstruction with trailing-literal flush (`FUN_08099050`), per-column RLE run-expander (`FUN_080acb90`, thr=1 for the LEN column / 0 for LIT·OFF), param14 text transform (`FUN_080a0ff0`), the bounded-varint header (`FUN_080b1dc0`), and the integrated chunk + whole-block decoders (`NzCdDecodeLzChunk`/`NzCdDecodeBlock`). Columns/literals reuse `DecodeArithBuffer`. The reconstruction runs over a **64 KB ring** (`FUN_08099050`, obj+0x978): each chunk's compact recon is written at the chunk's ring base (wrapping mod 65536) and matches resolve into prior chunks through the wrap, mirroring the binary's sliding window. **Wired into the extract dispatcher** and BYTE-EXACT end-to-end under `NZ_NO_BRIDGE=1` for: token-LZ (recon == file), **block-RLE** (flag `&2`, post-recon run-length re-expansion of collapsed zero-runs), **raw-store** (per-column `b0&1==0` and flag-`&1`-clear literals = verbatim bytes), **pure-literal** (no LZ tokens — the whole window is one literal stream; generator picks this when `size_field==0` or `v2==0`), **exe** (flag `&4`: a BCJ-style x86 E8/E9 call/jmp address un-transform, `NzCdExeUnfilter`), and the **text pipeline** (flag `&8`, `FUN_080a3c90`): param14 (`NzCdParam14`), line-RLE (`FUN_080a2f20`), CRLF EOL (`FUN_080a19b0`), and the **word-dictionary** transform (`FUN_080a0a00` = the reference `TransformText_1_Dictionary` + dict/char-trait tables, in `nz_cd_texttransform_dict.cpp`). Verified on text (map.txt), binary (image.cat), an ELF (elf.bin, flags=3 block-RLE), an EXE (play.exe, flags=5), atoll (multi-chunk `&8`), single-chunk word-dictionary text, and a varied real corpus spanning images/audio/video/executables. **Multi-chunk text still bridges**: a chunk following a text (`&8`) chunk references that chunk's expanded output and the cross-chunk ring advance for that case is not yet pinned (a known gap). The old `DecLZ` (`nz_lzhd.cpp`, nzdec_v0) is the wrong format for linux32 `-cd`. |
