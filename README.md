...
**Not there yet:** encode of `-cD/-co/-cO/-cc` and `s`, and self-extractor creation (`w32c`); `a -cn` (single and `-pN`), `a -cf/-cF` and `a -cd` (plain data, the exe and block-RLE filters, the text pipeline with its word dictionary, the audio and image models, under `-t1`) are byte-exact. `a` with one of the four unported compressors prints a one-line refusal and writes nothing (the decode phase's stub container, which the original could not read, is gone). Known limits: the
