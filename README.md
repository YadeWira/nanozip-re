...
**Not there yet:** encode of `-cc`; `a -co` and `a -cO` write plain-LZ, exe-filtered, text-transformed and BWT blocks, with the param1 and param2 post-filters, param14 on BWT blocks and stored (param6 == 0) BWT blocks, byte-identical to the original -- 292 of 294 corpus files for EACH of the two, the same two files differing in both, and none of the 294 declined -- but not yet writing the image or audio blocks, the stored LZ form, or budgets above 16 MB; a block it cannot write is declined with a message and no archive is left behind, and every block it does write is proved readable by our own decoder before it is committed; `a -cn` (single and `-pN`), `a -cf/-cF`, `a -cd` and `a -cD` (plain data, the exe and block-RLE filters, the text pipeline with its word dictionary, the audio and image models, under `-t1`, at every `-m`) are byte-exact. `a -cc` prints a one-line refusal and writes nothing (the decode phase's stub container, which the original could not read, is gone). Known limits: the
