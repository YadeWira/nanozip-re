...
| Every checksum setting (`-hn`, `-hc`, `-hC`, `-hf`, default) × 8 codecs × single and parallel containers (`tests/checksum_modes.sh`) | 240 checks: every extracted tree and every `t` run identical; 40 differ only in the ORDER a parallel container lists its files ([quirk 52](docs/ORIGINAL_QUIRKS.md)) |
| Damaged **parallel** containers, 8 codecs × 7 corruptions, single- and multi-file (`x`) | 104/112 identical trees (holes, short files, files created and left empty exactly where the original's workers leave them, [quirk 41](docs/ORIGINAL_QUIRKS.md)); report line identical in 101/112, the rest being `-cd`/`-cD` and `-cO` detection differences, one segfault of the original and the plain-vs-shifted code of four early failures |
