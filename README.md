...
**≈ 45/80 (~56%) byte-exact native, zero bridge.** The non-native cases fall to the
...
| lzhd decoder (`-cd/-cD`) | ⚠️ bridge (≈2/10 native); core decoders reverse-engineered | The real linux32 `-cd` is a coroutine token-LZ (NOT the reference `DecLZ`). Its core entropy decoders are now reverse-engineered and **validated byte-exact** against the binary: the **token assembler** (`FUN_080aa070`, 5346/5346 token fields exact) and the **token→bytes reconstruction** (`FUN_08099050`), both shipped in `nz_cd_tokens.{h,cpp}` as pure functions with regression tests (`TestCdReconstruct`/`TestCdTokenAssemble`). Literals (`DecodeArithBuffer`) + columns (arith/store+RLE) reuse existing ported pieces. **Still bridges end-to-end**: needs the block sub-stream framing and the tt08 dictionary expand wired into the dispatcher (the param14 space-insertion transform `FUN_080a0ff0` is now also ported+validated in `nz_cd_tokens.cpp`) (no unknown algorithms remain — see `ARCHITECTURE_cd.md`). The older `DecLZ` (`nz_lzhd.cpp`, from nzdec_v0) is faithful to that *reference* format but is the wrong format for linux32 `-cd`. |
