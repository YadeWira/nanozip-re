...
// The mirror of DecodeResidualsStereo (the residual coder the image model uses
// when its flag bit 0 is set). One byte per sample selects a bucket; the
// magnitude's low bits and the sign go to the side stream. The tables are the
// decoder's, read the other way: bucket i covers magnitudes
// [lo(i), lo(i) + 2^exbits(i)] and the byte is i + 1.
namespace {

struct StereoResidualTable {
    std::uint32_t lo[64];
    std::uint8_t bits[64];
    StereoResidualTable() {
        static const std::uint8_t kRange[64] = {
        ...
        static const std::uint8_t kSubOffset[64] = {
        ...
        static const std::uint8_t kExBits[64] = {
        ...
        static const std::uint32_t kBase[32] = {
        ...
        for (std::uint32_t i = 0; i < 64u; ++i) {
            const std::uint32_t r = kRange[i];
            const std::uint32_t e = kExBits[r];
            bits[i] = static_cast<std::uint8_t>(e);
            lo[i] = ((i - kSubOffset[r]) << (e & 0x1fu)) + kBase[r >> 1];
        }
    }
};

const StereoResidualTable& StereoTable() {
    static const StereoResidualTable t;
    return t;
}
...
void ResidualEncodeStereo(const std::int32_t* v, std::uint32_t n, std::uint8_t* bytes, BitWriter& w) {
    const StereoResidualTable& t = StereoTable();
    for (std::uint32_t i = 0; i < n; ++i) {
        const std::int32_t x = v[i];
        if (x == 0) { bytes[i] = 0; continue; }
        const std::uint32_t mag = static_cast<std::uint32_t>(x < 0 ? -static_cast<std::int64_t>(x)
                                                                   : static_cast<std::int64_t>(x));
        const std::uint32_t m1 = mag - 1u;
        std::uint32_t b = 0;
        for (std::uint32_t i2 = 64u; i2-- > 0;) {
            const std::uint32_t span = (t.bits[i2] >= 32u) ? 0xffffffffu : ((1u << t.bits[i2]) - 1u);
            if (m1 >= t.lo[i2] && m1 - t.lo[i2] <= span) { b = i2; break; }
        }
        bytes[i] = static_cast<std::uint8_t>(b + 1u);
        if (t.bits[b] != 0u) w.Put((m1 - t.lo[b]) & MaskBits(t.bits[b]), t.bits[b]);
        w.Put(x < 0 ? 1u : 0u, 1u);
    }
}
