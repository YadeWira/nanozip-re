...
// (NzOptimum2LzDecoder, src/nz_optimum2_lz.cpp) against golden vectors
// captured live from the real linux32/nz binary (GDB capture at the exact
// point FUN_080a5d90's caller returns, i.e. the LZ core's own direct
// input/output, before any outer text-transform/RLE postfilter). See
// include/nz_optimum2_lz.h and work/reports/decomp_optimum/
// optimum_lz_core_ARCHITECTURE.md for the full RE provenance.
//
// This MUST pass byte-exact on every fixture before the decoder is wired
// into sfx_archive.cpp's live dispatcher (mirrors test_optimum_lz.cpp's own
// convention/discipline).
#include "nz_optimum2_lz.h"
#include "nz_cm.h"  // NzCmInitAll() -- builds kLzModelInterpolation etc.,
                    // which nz_optimum2_lz.cpp's Stretch() reads.
...
int main(int argc, char** argv) {
    ...
    std::string dir = "tests/fixtures/optimum2_golden";
    if (argc > 1) dir = argv[1];

    RunFixture(dir, "aaa200_cO", 0x10000u);
    RunFixture(dir, "hientropy_cO", 0x30000u);
