...
| `encode/oracle.sh` | The encode oracle: 110 `a` invocations (`-cn` single and `-pN`, `-cf`/`-cF`, `-cd`; file order, `-r`, `-x`, unreadable files, every checksum kind, boundaries, audio, images, text shapes) run through the original under `-t1` and through `nz_recon`; archives byte-compared, each read by the other binary, consoles compared. Headline: 106/110 (the four left are the codecs not yet ported). |
